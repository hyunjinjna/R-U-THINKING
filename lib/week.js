...
/**
 * 13번 주말 리뷰 — 표시 기간과 "이번 주 완료된 회차" 계산
 * 기간: 토요일 0시 ~ 그 반의 다음 수업 시작 전 (휴강은 회차 계산이 자동 반영)
 * 대상: 직전 토요일(포함) ~ 이번 토요일(제외) 사이에 실제 진행된 회차들
 * @returns {null | { sessions: number[] }} 기간 밖이거나 완료 회차 없으면 null
 */
export function weekendReviewWindow(classData, today = getKoreaNow()) {
  ...
  const now = new Date(today);
  // 이번 주말의 "토요일 0시" = 가장 최근 토요일
  const sat = new Date(now);
  sat.setHours(0, 0, 0, 0);
  while (sat.getDay() !== 6) sat.setDate(sat.getDate() - 1);

  // 다음 수업 시작 시각 (토요일 이후 첫 수업일 + 수업시간)
  const cancellations = parseCancellations(classData['휴강기록']);
  ...
  if (now < sat || now >= nextClassStart) return null; // 표시 기간 밖

  // 직전 토요일 ~ 이번 토요일 사이에 진행된 회차 = countSessions 차이
  const prevSatEnd = new Date(sat);
  ...
  const prevSat2 = new Date(sat);
  prevSat2.setDate(prevSat2.getDate() - 8); // 직전 주 금요일
  const upToPrevFriday = countSessions(classData['시작일'], classData['수업요일'], classData['휴강기록'], prevSat2);

  if (upToFriday <= upToPrevFriday) return null; // 이번 주 진행된 회차 없음
  const sessions = [];
