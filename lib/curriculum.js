...
export function buildAutoHomework({ category, textbook, unit, sets, notesLink }) {
  ...
  if (cat === '리딩') {
    // 당일: 새 단어 암기·리콜 + 리딩 문제(사이트) + Workbook 드릴 / D+1: 지난 유닛 매칭 + 예문 말하기
    lines.push(line(`🆕 Unit ${n} 단어 암기·리콜 (두 바퀴 돌기!)`, unitSet(n)));
    lines.push(line(`📖 리딩 문제 풀기 (Unit ${n})`, `reading-quiz://${book}@${n}`));
...
/**
 * 13번 주말 리뷰 항목 생성
 * - 오답 복습: 세트 탭 "{교재} - 오답 복습" 줄의 링크 (교재=클래스카드 클래스 기준, 반마다가 아니라 교재마다 하나)
 * - 매칭 게임: 이번 주 완료된 유닛들의 단어세트 (문법은 단어세트가 없어 매칭 없음)
 * 링크를 못 찾으면 제목만 나감 — 세트 탭에 뭐가 빠졌는지 보이게 (자동 조립과 같은 원칙)
 */
export function buildWeekendReview({ category, textbook, units, sets, curriculumRows }) {
  if (!textbook || !units || units.length === 0) return [];
  const cat = String(category || '').trim();
  const items = [];

  const wrongLink = findSetLink(sets, `${textbook} - 오답 복습`);
  items.push({ title: '🔁 틀린 단어 다시 보기', link: wrongLink || '' });

  if (cat !== '문법') {
    for (const u of units) {
      const link = findSetLink(sets, `${textbook} - Unit ${u}`);
      items.push({ title: `🎮 Unit ${u} 매칭 게임`, link: link || '' });
    }
  }
  return items;
}

/** 커리큘럼 행들에서 회차 목록의 유닛 번호 뽑기 (중복 제거, 순서 유지) */
export function unitsOfSessions(curriculumRows, sessions, extractUnitFn) {
  const units = [];
  for (const s of sessions || []) {
    const row = (curriculumRows || []).find((r) => {
      const raw = r['회차'] || r['주차'] || r['차시'] || '';
      return parseInt(String(raw).replace(/[^0-9]/g, ''), 10) === s;
    });
    const u = row ? extractUnitFn(row['진도']) : null;
