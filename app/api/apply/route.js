...
export async function POST(request) {
  ...
  if (wrote === 0) {
    return Response.json({ ok: false, error: '기록할 열을 하나도 찾지 못했습니다.' });
  }

  const result = await appendRow(sheetId, tab, row);
  if (!result.ok) {
    return Response.json({ ok: false, error: result.error || '시트 기록에 실패했습니다.' });
  }

  // 새 문항 열(이전학습방식·걱정되는점)이 시트에 없으면 그 값은 조용히 빠짐 — 코치 화면에는 영향 없음
  const missingNew = ['이전학습방식', '걱정되는점'].filter((k) => values[k] && findCol(headers, k) < 0);

  return Response.json({ ok: true, missingColumns: missingNew });
}
